...
function ImageToPdf() {
  ...
  const handleExport = async () => {
    console.log('[ImageToPdf] handleExport called, items:', items.length);
    if (items.length === 0) {
      setError('请先添加图片');
      return;
    }
...
function PdfToImages() {
  ...
  const handleExport = async () => {
    ...
    setProgress(0);
    try {
      const pages = parseRange(pageRange, pageCount);
      ...
      <Box sx={{ flex: 1, minWidth: 0, width: '100%' }}>
        {!pdfFile ? (
          ...
          >
            ...
            <Button variant="contained" size="small" component="label" sx={{ mt: 1 }} startIcon={<UploadFileIcon sx={{ fontSize: 16 }} />}>
              选择 PDF
    ...
    <Box>
      <Stack direction="row" spacing={1.5} sx={{ mb: 2, alignItems: 'center', flexWrap: 'wrap', gap: 1 }}>
        <ToggleButtonGroup
          ...
          exclusive
          size="small"
          onChange={(_, v) => v && setDir(v as Dir)}
        >
          <ToggleButton value="i2p" sx={{ px: 2, py: 0.5, fontSize: 13 }}>
            图片 → PDF
          </ToggleButton>
          <ToggleButton value="p2i" sx={{ px: 2, py: 0.5, fontSize: 13 }}>
            PDF → 图片
          </ToggleButton>
        </ToggleButtonGroup>
      </Stack>

      {dir === 'i2p' ? <ImageToPdf /> : <PdfToImages />}
    </Box>
  );
}